...
namespace Physics
{
	...
	bool ParallelogramHitbox::contains(const glm::vec3& point, glm::vec3 thisPosition) const
	{
		...
		return m_halfWidth > std::abs(delta.x) &&
			m_halfHeight > std::abs(delta.y) &&
			m_halfDepth > std::abs(delta.z);
	}
	...
	{
		if (para.m_halfHeight + cyl.m_halfHeight <= std::abs(paraPosition.y - cylPosition.y))
			return 0;
		...
		const ParallelogramHitbox& para2, glm::vec3 para2Position)
	{
		...
		return para1.m_halfWidth + para2.m_halfWidth > std::abs(delta.x) &&
			para1.m_halfHeight + para2.m_halfHeight > std::abs(delta.y) &&
			para1.m_halfDepth + para2.m_halfDepth > std::abs(delta.z);
		...
		const CylinderHitbox& cyl2, glm::vec3 cyl2Position)
	{
		...
		return delta.x * delta.x + delta.z * delta.z < std::pow(cyl1.m_radius + cyl2.m_radius, 2) &&
			cyl1.m_halfHeight + cyl2.m_halfHeight > std::abs(delta.y);
	}
