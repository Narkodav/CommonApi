...
namespace Utilities {
	...
	public:
		...
		public:
			using EventSystemType = EventSystem<Policy>;
		...
		static inline const bool s_isScoped = false;
	...
	public:
		...
		template<Policy::Type T>
		Subscription<T> subscribe(Subscriber<T> callback)
		{
			auto id = getId();
			SubscriptionInternal<T> internal = { id, callback };
			std::get<std::vector<SubscriptionInternal<T>>>(m_subscribers).push_back(internal);
			return id;
		}

		template<Policy::Type T, typename Handler>
		Subscription<T> subscribe(auto callback, Handler& handler)
		{
		...
		{
			const auto& subscribers = std::get<std::vector<SubscriptionInternal<T>>>(m_subscribers);
		...
		EventSystem& unsubscribe(const Subscription<T>& id) {
			auto& subscribers = std::get<std::vector<SubscriptionInternal<T>>>(m_subscribers);
