...
namespace Utilities {
	...
	public:
		...
		static inline const bool s_isScoped = true;
	...
	public:
		...
		{
			std::get<std::vector<Subscriber<T>>>(m_subscribers).push_back(callback);
			return *this;
		}
		...
		ScopedEventSystem& subscribe(auto callback, Handler& handler)
		{
			...
			std::get<std::vector<Subscriber<T>>>(m_subscribers).push_back(wrappedSubscriber);
			return *this;
		}

		template<Policy::Type T, typename... Args>
		ScopedEventSystem& emit(Args&&... args) const
		{
			const auto& subscribers = std::get<std::vector<Subscriber<T>>>(m_subscribers);
