...
namespace Utilities {
	...
	public:
		...
		template<Policy::Type T>
		SingleCallbackEventSystem& set(const Callback<T>& callback)
		{
			std::get<static_cast<size_t>(T)>(m_callbacks) = callback;
			return *this;
		}

		template<Policy::Type T, typename Handler>
		SingleCallbackEventSystem& set(auto callback, Handler& handler)
		{
			Callback<T> wrappedCallback = [&handler, callback](auto&&... args) {
